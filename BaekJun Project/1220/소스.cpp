...
int main() {
	...
	int xa, ya, xb, yb, n;
	scanf_s("%d %d %d %d %d", &xa, &ya, &xb, &yb, &n);

	vector<int> x1(n), y1(n), x2(n), y2(n), t(n);

	for (int i = 0; i < n; ++i)
		scanf_s("%d %d %d %d %d", &x1[i], &y1[i], &x2[i], &y2[i], &t[i]);

	set<int> xs, ys;
	xs.insert(xa), xs.insert(xb);
	ys.insert(ya), ys.insert(yb);

	for (int i = 0; i < n; ++i) {

		xs.insert(x1[i]), xs.insert(x2[i]);
		ys.insert(y1[i]), ys.insert(y2[i]);
